...
template <class T>
void InsertThroughputTest(BenchmarkEnvironment<T, thread_num> *env) {
    std::vector<std::thread> threads;
    size_t keys_per_thread = env->numkeys * ((end_load-begin_load) / 100.0) / thread_num;
    timeval t1, t2;
    gettimeofday(&t1, NULL);
    for (size_t i = 0; i < thread_num; i++) {
        threads.emplace_back(inserter<T, thread_num>::fn, std::ref(env->table),
...
int main(int argc, char** argv) {
    const char* args[] = {"--power", "--begin-load", "--end-load", "--seed"};
    size_t* arg_vars[] = {&power, &begin_load, &end_load, &seed};
