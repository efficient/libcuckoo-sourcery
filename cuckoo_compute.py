...
while 1:
    ...
    if not line:
        break
    if line.startswith('cuckoo_compute'):
...
print 'all:  num = %d, max = %d, mean = %.3f %d %d %d %d %d'%\
    (len(path_len),  max(path_len), np.mean(path_len),\
...
for i in range(0,5):
    print 'seg %d: num = %d, max = %d, mean = %.3f %d %d %d %d %d'%\
    (i, len(pLen[i]),  max(pLen[i]), np.mean(pLen[i]), \
